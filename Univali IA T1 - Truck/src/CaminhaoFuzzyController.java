public class CaminhaoFuzzyController {
	...
	// Recuperação de colisão — não fazem parte da lógica de estacionamento
	private int    tempoRecuando   = 0;   // recua após bater indo para frente
	private int    tempoAvancando  = 0;   // avança após bater indo de ré
	private double ultimaVelocidade = 0;  // rastreia direção do último movimento
	...
	public Controle calcular(double x, double y, double ang, boolean colidiu, int diffTime) {
		...
		// Recuperação de colisão: escolhe direção oposta ao movimento que causou a batida
		if (colidiu) {
			if (ultimaVelocidade < 0) {
				// Bateu indo de ré → avança para sair
				tempoAvancando  = 120;
				tempoRecuando   = 0;
			} else {
				// Bateu indo para frente → recua para sair
				tempoRecuando   = 60;
				tempoAvancando  = 0;
			}
		}
		...
		if (tempoAvancando > 0) {
			tempoAvancando  -= diffTime;
			ultimaVelocidade = 45;
			// Avança e gira em direção ao centro (oposto do recuando)
